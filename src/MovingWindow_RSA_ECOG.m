function [results,fitObj] = MovingWindow_RSA_ECOG(condition, subjects, holdouts, varargin)
...
% 2. sqrt_truncate_r.m (from WholeBrain_RSA/src)
    ...
    if isdeployed
        ...
        holdout_set = str2double(strsplit(p.Results.holdouts));
        disp(p.Results)
        windows_set = str2double(strsplit(p.Results.windows));
    ...
    load('cvpartition_10fold_leuven_byanimate.mat', 'CV');
